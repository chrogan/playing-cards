...
public class Trick {
  ...
  private static void tallyPiles(List<Card> blackPile, List<Card> redPile) {
    int blackCount = (int) blackPile.stream()
        .filter((card) -> card.getSuit().getColor()==Color.BLACK)
        .count();
    blackPile.sort(
        Comparator.comparing
                (Card::getSuit, Comparator.comparing(Suit::getColor))
            .thenComparing(Comparator.naturalOrder())
    );
    int redCount = (int) redPile.stream()
        .filter((card) -> card.getSuit().getColor()==Color.RED)
        .count();
    redPile.sort(
        Comparator.comparing
                (Card::getSuit, Comparator.comparing(Suit::getColor).reversed())
            .thenComparing(Comparator.naturalOrder())
    );
